...
public class Array16 {
	...
	public static void main(String[] args) {
		...
		System.out.println();
		a[2] =100;   // --- 여기서 b도 왜 바뀌냐? 복사가 아니라 b변수에다가 a주소를 주네!! 와!!!
		for (int k : a) System.out.printf("%d ",k);
		System.out.println();
		for (int k : b) System.out.printf("%d ", k);
		System.out.println();
		...
		// 따라서 a[2]값 변경은b[2] 값 변경과 같습니다.
		
		// 1. 배열복사의 방법 1번 : 새로운 공간을 new 로 만들고 각 요소를 일일이 복사 --- 이생각 밖에 안났음 이걸 좋은 방법이 있나?
		b = new int[5];
		...
		int[][] arr5 = arr4.clone();
		// arr4과 공유하고 있는 1차원 배열의 값이 수정
		// 얕은 복사로 복사된 arr5의 값을 수정하게 되면 arr4도 변경된 값을 출력하게됨
		arr4[1][0] =400;
		System.out.println("arr5(arr clone) : ");
		for(int i = 0; i<arr4.length ; i++) {
			for(int j = 0 ; j <arr4[i].length ; j++)
		...
		// 결국 이렇게해야한다
		arr5[0] = new int[3];  // --- 이게 아니라 arr5[0] = arr4[0].clone(); 이렇게 
		arr5[1] = new int[3];  // --- 이해하려면 clone 메소드를 뜯어봐된다 내가보기엔
